...
def replay_plan(
    ...
    """Re-check energy balance, battery rules, and every applied directive."""
    if len(plan) != 24:
    ...
    for entry, source in zip(plan, ordered):
        if entry.hour != source.hour:
        ...
        if entry.hour in no_charge and charge > TOLERANCE:
            raise RuntimeError("charge occurred inside a no_charge_window")
        if entry.hour in no_discharge and discharge > TOLERANCE:
            raise RuntimeError("discharge occurred inside a no_discharge_window")
        if entry.solar_used_kwh - solar[entry.hour] > TOLERANCE:
            raise RuntimeError("solar_used_kwh exceeds effective solar")
        if max_grid[entry.hour] is not None and entry.grid_kwh - max_grid[entry.hour] > TOLERANCE:
            raise RuntimeError("grid_kwh exceeds max_grid_window cap")

        if abs(entry.grid_kwh + entry.solar_used_kwh + discharge - source.demand_kwh - charge) > TOLERANCE:
        ...
        if entry.battery_energy_after_kwh + TOLERANCE < min_soc[entry.hour]:
